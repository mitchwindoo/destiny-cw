...
clan_bonus AS (SELECT CASE WHEN COUNT(*) = 1 THEN 1 WHEN COUNT(*) > 1 THEN COUNT(*) * 0.05 + 1 END AS value, players.destinyid FROM activities INNER JOIN players ON ( activities.playerdestinyid = players.destinyid ) GROUP BY players.destinyid),
