-- getLargeClanActivityScore
-- Get the Current Event Mode ID
WITH current_event AS (
		SELECT mode,start_date,end_date
		FROM events
		WHERE CURRENT_TIMESTAMP - INTERVAL '1 DAY' BETWEEN events.start_date AND events.end_date
), 
-- Calculate Clan Bonus
clan_bonus AS (SELECT CASE WHEN COUNT(*) = 1 THEN 1 WHEN COUNT(*) > 1 THEN COUNT(*) * 0.05 + 1 END AS value, clanid FROM activities INNER JOIN players ON ( activities.playerdestinyid = players.destinyid ) GROUP BY players.clanid),
